...
mod kintsugi_imports {
	...
	pub const NATIVE_CURRENCY: CurrencyId = Token(KINT);
	pub const RELAY_CHAIN_CURRENCY: CurrencyId = Token(KSM);

	pub use primitives::{KINT, KSM, CurrencyId::Token};
}
...
impl ExtBuilder {
	...
	pub fn build(self) -> sp_io::TestExternalities {
		...
		let existential_deposit = ExistentialDeposits::get(&native_currency_id);
...
pub fn alice() -> AccountId {
	AccountId::from(Into::<[u8; 32]>::into(alice_key().serialize()))
}

pub fn bob() -> AccountId {
	let address = bob_key().serialize();
	AccountId::from(Into::<[u8; 32]>::into(alice_key().serialize()))
}
